...
Normal maps визначаються шляхом сканування `.MAT` файлів матеріалів за записами `Texture1_BUMP:`. Інструмент автоматично знаходить тецю `MATERIALS/` — ніякого ручного налаштування не потрібно:

| Місце пошуку | Примітка |
|---|---|
| `<корінь гри>/MATERIALS/` | Патч-оверрайди гри (вищий пріоритет) |
| `<корінь гри>/GrpUnpacker/MATERIALS/` | Базові дані з GRP, розпакованi GrpUnpacker |

Корінь гри визначається автоматично за вказаними шляхами (вхідна папка та розташування VELoader). Якщо жодна з `MATERIALS/` директорій не знайдена — використовується евристика за суфіксом `_N` / `_N_` у назві файлу.
